/**
 * Vercel Cron Wrapper for Orchestrator
 *
 ...
 *
 * Architecture:
 * Vercel Cron → /api/cron-wrapper/orchestrator (this file)
 *            → /api/cron/orchestrator (actual logic with full security)
 *            → Individual task endpoints (/api/cron/poll-costs, etc.)
 */
...
export async function GET(request: NextRequest) {
  ...
  const method = 'GET'
  const path = '/api/cron/orchestrator'
  const body = ''
  ...
  try {
    ...
    if (!response.ok) {
      console.error('[Cron Wrapper] Orchestrator error:', data)
      return NextResponse.json(data, { status: response.status })
    }

    return NextResponse.json(data)
  } catch (error: any) {
    logSecureError('CronWrapper:Orchestrator', error, {
      endpoint: 'GET /api/cron-wrapper/orchestrator'
    })
    return createSecureErrorResponse('Failed to execute orchestrator', 500)
  }
}

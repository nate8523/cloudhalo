...
# Test script for alert evaluation cron job with HMAC signing
# Usage: ./scripts/test-alert-cron.sh [environment]
# Example: ./scripts/test-alert-cron.sh local
#          ./scripts/test-alert-cron.sh production
...
echo ""
echo -e "${BLUE}=== Testing Alert Evaluation Cron Job ===${NC}"
echo ""

# Test 1: Call the cron endpoint with HMAC signature
echo -e "${YELLOW}[1/5] Calling alert evaluation cron endpoint with HMAC signature...${NC}"
...
RESPONSE=$($CURL_CMD -s -w "\n%{http_code}" -X GET \
    "${BASE_URL}/api/cron/evaluate-alerts" \
...
echo ""
echo -e "${YELLOW}[2/5] Checking other cron endpoints with HMAC...${NC}"

# Test other cron jobs for completeness
CRON_JOBS=(
    "poll-costs:GET"
    "poll-resources:GET"
    "send-reports:POST"
...
for job_config in "${CRON_JOBS[@]}"; do
    IFS=':' read -r job method <<< "$job_config"
    echo -e "${BLUE}  Testing /api/cron/${job} (${method})...${NC}"

    # Generate fresh timestamp and signature for each request
    TIMESTAMP=$($DATE_CMD -u +"%Y-%m-%dT%H:%M:%SZ")
    PATH="/api/cron/${job}"
    BODY=""
    SIGNATURE=$(generate_hmac_signature "$method" "$PATH" "$TIMESTAMP" "$BODY" "$CRON_SECRET")

    HTTP_CODE=$($CURL_CMD -s -o /dev/null -w "%{http_code}" -X "$method" \
    ...
    if [ "$HTTP_CODE" -eq 200 ]; then
        echo -e "${GREEN}  ✓ ${job} endpoint is accessible${NC}"
    else
        echo -e "${YELLOW}  ⚠ ${job} endpoint returned HTTP ${HTTP_CODE}${NC}"
    fi
done

echo ""
echo -e "${YELLOW}[3/5] Verifying alert evaluation metrics...${NC}"

# Parse the response to check metrics
EVALUATED=$(echo "$BODY" | jq -r '.evaluated' 2>/dev/null || echo "N/A")
TRIGGERED=$(echo "$BODY" | jq -r '.triggered' 2>/dev/null || echo "N/A")
TIMESTAMP=$(echo "$BODY" | jq -r '.timestamp' 2>/dev/null || echo "N/A")

echo -e "${BLUE}  Alert Rules Evaluated: ${GREEN}${EVALUATED}${NC}"
echo -e "${BLUE}  Alerts Triggered: ${GREEN}${TRIGGERED}${NC}"
...
EXPIRED_SIGNATURE=$(generate_hmac_signature "GET" "/api/cron/evaluate-alerts" "$EXPIRED_TIMESTAMP" "" "$CRON_SECRET")
...
WRONG_SIGNATURE=$(generate_hmac_signature "GET" "/api/cron/evaluate-alerts" "$WRONG_TIMESTAMP" "" "wrong_secret")
...
# Summary
if [ "$TRIGGERED" != "0" ] && [ "$TRIGGERED" != "N/A" ]; then
    echo -e "${YELLOW}⚠ Note: ${TRIGGERED} alert(s) were triggered. Check your email and alert_history table.${NC}"
elif [ "$EVALUATED" != "0" ] && [ "$EVALUATED" != "N/A" ]; then
    echo -e "${GREEN}✓ ${EVALUATED} alert rule(s) evaluated successfully (no alerts triggered).${NC}"
else
    echo -e "${BLUE}ℹ No active alert rules found to evaluate.${NC}"
fi

echo ""
echo -e "${BLUE}Next steps:${NC}"
echo "  1. Check server logs for detailed evaluation output"
echo "  2. Query alert_history table to see triggered alerts"
